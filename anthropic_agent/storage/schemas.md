...
CREATE TABLE agent_config (
    ...
    description TEXT,  -- Short user-facing description (used by SubAgentTool for tool schemas)
    ...
    -- Subagent hierarchy (for nested agent orchestration via SubAgentTool)
    parent_agent_uuid UUID,  -- UUID of the parent agent that spawned this child (NULL for root agents)
    subagent_schemas JSONB DEFAULT '[]',  -- [{name, description, model}, ...] registered subagents
...
CREATE INDEX idx_agent_config_parent ON agent_config(parent_agent_uuid) WHERE parent_agent_uuid IS NOT NULL;
```
...
# Stored as: data/agent_config/{agent_uuid}.json
{
  ...
  "description": "Researches topics and provides analysis",  # Short user-facing description (nullable)
  ...
  # Subagent hierarchy (for nested agent orchestration via SubAgentTool)
  "parent_agent_uuid": "parent-uuid-789",  # UUID of parent agent (null for root agents)
  "subagent_schemas": [
    # Registered subagent identifiers and descriptions
    {"name": "researcher", "description": "Researches topics", "model": "claude-sonnet-4-5"},
    {"name": "coder", "description": "Writes and modifies code", "model": "claude-sonnet-4-5"}
  ],
...
### Pattern 6: Find all child agents of a parent (Subagent Hierarchy)
```sql
SELECT agent_uuid, description, model, parent_agent_uuid, subagent_schemas
FROM agent_config
WHERE parent_agent_uuid = 'parent-uuid-789';
```

### Pattern 7: Get subagent tree for a root agent
```sql
-- Recursive CTE to walk the full subagent hierarchy
WITH RECURSIVE agent_tree AS (
    SELECT agent_uuid, description, parent_agent_uuid, 0 AS depth
    FROM agent_config
    WHERE agent_uuid = 'root-uuid-123'
  UNION ALL
    SELECT c.agent_uuid, c.description, c.parent_agent_uuid, t.depth + 1
    FROM agent_config c
    JOIN agent_tree t ON c.parent_agent_uuid = t.agent_uuid
)
SELECT * FROM agent_tree ORDER BY depth, agent_uuid;
```
