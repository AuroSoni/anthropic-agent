...
def _get_json_schema_type(param_type: type) -> dict[str, str]:
    ...
    if str(param_type) == "Tensor":
        ...
        if torch_spec:
            ...
            if tensor_type and param_type == tensor_type:
                return {"type": "audio"}
    return {"type": "object"}
...
def get_json_schema(func: callable) -> dict:
    ...
    `(choices: ["tea", "coffee"])`, which will be parsed into an `enum` field in the schema. Note that this
    will only be parsed correctly if it is at the end of the line:
...
def generate_anthropic_schema(func: Callable) -> dict:
    """Generate an Anthropic-compliant tool schema from a function.
    
    This function analyzes a Python function's docstring (Google-style preferred)
    and type hints to automatically generate a tool schema in Anthropic's format.
    """
